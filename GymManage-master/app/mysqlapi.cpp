...
void MysqlApi::BindData(QString tableName, QString orderColumn, QString orderType, int rowsCount, QString columnNames[], int columnWidths[])
{
    ...
    TempSql = "select venueNo,Typename,Typrice,venueState,venueRemark from "+tableName+" where venue.venueTypeId=venuetype.venueTypeId order by "+orderColumn+" "+orderType;
